...
namespace Aplicacion.Integration.Tests.Comun
{
    ...
    public class SliceFixture : IAsyncLifetime
    {
        ...
        public SliceFixture()
        {
            this.factory = new AplicationFactory();
            ...
            this.dbconnection = Environment.GetEnvironmentVariable("URI_DB")!;
        }

        public async Task InitializeAsync()
        {
            var checkpoint = await Respawner.CreateAsync(this.dbconnection!);
            await checkpoint.ResetAsync(this.dbconnection!);
        }
        ...
        public async Task<T> ExecuteScopeAsync<T>(Func<IServiceProvider, Task<T>> action)
        {
            ...
            var dbContext = scope.ServiceProvider.GetService<Contexto>();
        ...
        public Task<T> ExecuteDbContextAsync<T>(Func<Contexto, Task<T>> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<Contexto>()));

        public Task<T> ExecuteDbContextAsync<T>(Func<Contexto, IMediator, Task<T>> action)
            => ExecuteScopeAsync(sp => action(sp.GetService<Contexto>(), sp.GetService<IMediator>()));
        ...
        public Task<Contexto> Context() => ExecuteScopeAsync(sp => Task.FromResult(sp.GetService<Contexto>()));
    }
}
